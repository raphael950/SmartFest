...
export default class RaceController {
  async index({ inertia }: HttpContext) {
    ...
    const raceState = socketService.getRaceState()

    return inertia.render('race/race-management', {
  ...
  async startRace({ response }: HttpContext) {
    await socketService.startRace()
    return response.redirect().back()
  }

  async stopRace({ response }: HttpContext) {
    await socketService.stopRace()
    return response.redirect().back()
  }
